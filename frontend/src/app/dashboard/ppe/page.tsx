...
import { Code2, Play, Sparkles, CheckCircle2, XCircle, Loader2, Lightbulb, Clock, Send } from 'lucide-react';
...
export default function PPEPage() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      push('success', 'Submitted for AI evaluation');
    } catch (err: any) {
      push('error', err?.message || 'Submission failed');
    } finally {
      setSubmitting(false);
    }
  };
    ...
    <div className="space-y-4" style={{ height: 'calc(100vh - 140px)' }}>
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div className="flex items-center gap-2 flex-wrap">
          ...
          {startedAt != null && (
            <span className="inline-flex items-center gap-1 text-xs text-gray-500 dark:text-gray-400 ml-2 tabular-nums" aria-label="Elapsed time">
              <Clock className="h-3 w-3" aria-hidden="true" /> {formatTime(elapsedMs)}
            </span>
          )}
        </div>
        <div className="flex items-center gap-2">
          <label className="sr-only" htmlFor="ppe-problem">Select problem</label>
          <select
            id="ppe-problem"
            value={selected?.id || ''}
            onChange={(e) => {
          ...
          <label className="sr-only" htmlFor="ppe-language">Language</label>
          <select
            id="ppe-language"
            value={language}
      ...
      {!error && problems.length > 0 && (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4" style={{ height: 'calc(100% - 60px)' }}>
          <div className="bg-white dark:bg-surface-900 rounded-xl border border-gray-200 dark:border-surface-700 p-5 overflow-y-auto">
            {selected ? (
              <div className="space-y-3">
                ...
                {selected.examples && Array.isArray(selected.examples) && selected.examples.length > 0 && (
                  <div>
                    <h4 className="text-xs font-bold uppercase tracking-wider text-gray-500 dark:text-gray-400 mb-2">Examples</h4>
                    <div className="space-y-2">
                      {selected.examples.map((ex: any, i: number) => (
                        <div key={i} className="p-2.5 bg-gray-50 dark:bg-surface-800 rounded-lg font-mono text-xs">
                          <div><span className="text-gray-500 dark:text-gray-400">Input:</span> {JSON.stringify(ex.input)}</div>
                          <div><span className="text-gray-500 dark:text-gray-400">Output:</span> {JSON.stringify(ex.output)}</div>
                ...
                {selected.constraints && (
                  <div>
                    <h4 className="text-xs font-bold uppercase tracking-wider text-gray-500 dark:text-gray-400 mb-2">Constraints</h4>
          ...
          <div className="bg-white dark:bg-surface-900 rounded-xl border border-gray-200 dark:border-surface-700 flex flex-col overflow-hidden">
            ...
            <textarea
              ...
              className="flex-1 resize-none bg-gray-900 p-4 font-mono text-sm text-green-400 focus:outline-none focus:ring-2 focus:ring-inset focus:ring-blue-500"
            ...
            {(result || aiFeedback) && (
              <div className="border-t border-gray-200 dark:border-surface-700 bg-gray-50 dark:bg-surface-800 max-h-56 overflow-y-auto">
                <div className="px-4 py-2 flex items-center justify-between sticky top-0 bg-gray-50 dark:bg-surface-800 border-b border-gray-200 dark:border-surface-700 z-10">
                  <span className="text-xs font-semibold text-gray-700 dark:text-gray-200">Result</span>
                  <div className="flex items-center gap-2">
                ...
                <div className="px-4 py-2 space-y-1">
                  {Array.isArray(result?.test_results) && result.test_results.map((t: any, i: number) => (
                    <div key={i} className={`flex items-center gap-2 text-xs ${t.passed ? 'text-green-700 dark:text-green-400' : 'text-red-700 dark:text-red-400'}`}>
                      ...
                      {!t.passed && t.expected !== undefined && (
                        <span className="text-gray-500 dark:text-gray-400">— expected {JSON.stringify(t.expected)}, got {JSON.stringify(t.actual)}</span>
                      )}
                    </div>
                  ))}
                  ...
                  {result?.output && (
                    <pre className="mt-2 p-2.5 bg-gray-900 text-green-400 font-mono text-xs rounded overflow-x-auto" aria-label="Code output">{result.output}</pre>
                  )}
                </div>
              </div>
            )}
