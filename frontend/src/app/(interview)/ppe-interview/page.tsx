...
const PROBLEMS = [
  { id: 'two-sum', title: 'Two Sum', difficulty: 'easy', description: 'Given an array of integers nums and an integer target, return indices of the two numbers such that they add up to target.', examples: [{ input: 'nums = [2,7,11,15], target = 9', output: '[0,1]' }], constraints: ['2 <= nums.length <= 10^4'], starterCode: { python: 'def two_sum(nums, target):\n    # Your solution here\n    pass\n' } },
];

export default function PPEInterviewPage() {
  ...
  const handleRun = () => {
    ...
    setTimeout(() => {
      ...
      setAgentMessages(prev => [...prev, { role: 'agent', content: '3/5 tests pass. Consider edge cases.' }]);
      setIsRunning(false);
    }, 1500);
  };
  ...
  const handleHint = () => {
    ...
    const hints = ['Try using a hash map.', 'Iterate once.', 'Map seen values to indices.'];
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-4">
      <div className="flex items-center justify-between mb-4">
        <div>
          <h1 className="text-2xl font-bold">PPE Interview</h1>
          <p className="text-sm text-gray-500">AI-powered pair programming evaluation</p>
        </div>
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-5" style={{ height: 'calc(100vh - 120px)' }}>
                    ...
                    <div className="space-y-2">
                      ...
                      {executionResult.test_results?.map((t: any) => (
                        <div key={t.test} className={`rounded-lg border p-2 text-sm ${t.passed ? 'border-green-200 bg-green-50' : 'border-red-200 bg-red-50'}`}>
                          Test {t.test} {t.passed ? '✓' : '✗'}
                        </div>
                      ))}
                    </div>
                  ) : <p className="text-sm text-gray-500 text-center py-8">Run code to see results</p>}
                </div>
              )}
              {activeTab === 'chat' && (
                <div className="flex flex-col h-full">
                  <div className="flex-1 space-y-3 overflow-y-auto">
                    {agentMessages.map((msg, i) => (
                      <div key={i} className={`rounded-lg p-3 text-sm ${msg.role === 'agent' ? 'bg-blue-50' : 'bg-gray-100 ml-4'}`}>
                        <p className="font-medium text-xs mb-1">{msg.role === 'agent' ? 'AI' : 'You'}</p>
                        <p>{msg.content}</p>
                      </div>
                    ))}
                  </div>
                  <div className="flex gap-2 mt-3">
                    <input value={chatInput} onChange={e => setChatInput(e.target.value)}
                      ...
                      placeholder="Ask AI..." className="flex-1 rounded-lg border px-3 py-2 text-sm focus:outline-none" />
                    <button onClick={handleSendChat} className="rounded-lg bg-blue-600 px-3 py-2 text-white text-sm">Send</button>
                  </div>
                </div>
              )}
            </div>
        ...
        <div className="lg:col-span-3 flex flex-col gap-4">
          <div className="flex-1 bg-white rounded-xl border flex flex-col">
            <div className="flex items-center justify-between border-b px-4 py-2">
              ...
              <select value={language} onChange={e => setLanguage(e.target.value)} className="rounded border px-2 py-1 text-xs">
          ...
          <div className="flex items-center gap-3">
            ...
            <a href="/dashboard" className="ml-auto text-sm text-gray-500 hover:text-gray-700">← Back to Dashboard</a>
          </div>
        </div>
      </div>
    </div>
