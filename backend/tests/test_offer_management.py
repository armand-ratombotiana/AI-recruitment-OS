...
Covers:
* Creating offers (with title, currency, benefits)
* Listing offers
* Updating draft offers
* Sending offers
* Accepting offers
* Declining offers (with declined_at)
* Signing offers (e-signature with OfferSignature record)
* Offer templates CRUD (including update)
* Tenant isolation
...
async def offer_client(engine):
    from apps.offer_management.main import router as offer_router
...
async def test_create_offer(offer_client, seed_factory):
    ...
    assert body["title"] == "Software Engineer Offer"
    assert body["salary"] == 120000
    assert body["currency"] == "USD"
    assert body["equity"] == 0.5
    assert body["benefits"] == {"health": True, "dental": True}
...
async def test_update_draft_offer(offer_client, seed_factory):
    ...
    resp = await offer_client.put(
        f"/api/v1/offers/{oid}",
        headers=seed["headers"],
        json={"salary": 100000, "title": "Updated Title", "terms": {"signing_bonus": 5000}},
    )
    assert resp.status_code == 200
    body = resp.json()
...
@pytest.mark.asyncio
async def test_update_template(offer_client, seed_factory):
    seed = await seed_factory()
    r = await offer_client.post(
        "/api/v1/offers/templates",
        headers=seed["headers"],
        json={"name": "Old Name", "content": "Old content"},
    )
    tid = r.json()["id"]
    resp = await offer_client.put(
        f"/api/v1/offers/templates/{tid}",
        headers=seed["headers"],
        json={"name": "New Name", "content": "New content", "variables": {"x": "y"}},
    )
    assert resp.status_code == 200
    body = resp.json()
...
@pytest.mark.asyncio
async def test_update_template_not_found(offer_client, seed_factory):
    seed = await seed_factory()
    resp = await offer_client.put(
        "/api/v1/offers/templates/nonexistent",
        headers=seed["headers"],
        json={"name": "Nope"},
    )
    assert resp.status_code == 404
...
async def test_full_offer_lifecycle(offer_client, seed_factory):
    ...
    assert r.json()["title"] == "Full Lifecycle Offer"
