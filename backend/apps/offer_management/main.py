"""Offer Management Service — CRUD, lifecycle, e-signatures, and templates."""
...
from shared.core.models.offer import Offer, OfferStatus, OfferTemplate, OfferSignature
...
def _offer_to_response(offer: Offer) -> OfferResponse:
    return OfferResponse(
        ...
        equity=offer.equity,
        benefits=_parse_json(offer.benefits),
        start_date=offer.start_date,
        expiration_date=offer.expiration_date,
        terms=_parse_json(offer.terms),
...
def _template_to_response(tpl: OfferTemplate) -> TemplateResponse:
    return TemplateResponse(
        ...
        name=tpl.name,
        content=tpl.content,
        variables=_parse_json(tpl.variables),
...
@router.put("/templates/{template_id}", response_model=TemplateResponse, tags=["Offers"])
async def update_template(
    ...
    result = await db.execute(
        select(OfferTemplate).where(OfferTemplate.id == template_id, OfferTemplate.tenant_id == tenant_id)
    )
    ...
    if "variables" in update_data and update_data["variables"] is not None:
        tpl.variables = json.dumps(update_data.pop("variables"))
    else:
        update_data.pop("variables", None)
...
async def update_offer(
    ...
    if "benefits" in update_data and update_data["benefits"] is not None:
        offer.benefits = json.dumps(update_data.pop("benefits"))
    else:
        update_data.pop("benefits", None)
...
async def decline_offer(
    ...
    offer.declined_at = datetime.now(timezone.utc).replace(tzinfo=None)
    db.add(offer)
    await db.flush()
    await db.refresh(offer)
    return _offer_to_response(offer)
...
async def sign_offer(
    ...
    signature = OfferSignature(
        offer_id=offer_id,
        signer_name=data.signer_name or "Unknown",
        signer_email=data.signer_email or "unknown@example.com",
