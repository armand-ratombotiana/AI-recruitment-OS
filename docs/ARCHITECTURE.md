...
|---------|------|-------------|
| API Gateway | 8000 | Unified FastAPI gateway routing to all services |
| Auth Service | 8000 | Authentication, JWT, MFA |
| Tenant Service | 8000 | Multi-tenant management |
| User Service | 8000 | User CRUD and management |
| Candidate Service | 8000 | Candidate profiles and enrichment |
| Resume Service | 8000 | Resume upload, parsing, embedding |
| Job Service | 8000 | Job posting and management |
| Interview Service | 8000 | Interview scheduling and orchestration |
| PPE Service | 8000 | Pair programming evaluation |
| AI Orchestrator | 8000 | Multi-agent AI coordination |
| Analytics Service | 8000 | Metrics, dashboards, reporting |
| Workflow Engine | 8000 | No-code workflow automation |
| Notification Service | 8000 | Multi-channel notifications |
| Compliance Service | 8000 | GDPR, audit, data retention |
| Billing Service | 8000 | Subscriptions and usage tracking |
| Vector Search Service | 8000 | Semantic search and embeddings |
...
|-------|---------|----------|
| Resume Parsing | Extract structured data from resumes | Resume upload |
| Candidate Profiling | Build comprehensive candidate profiles | Resume parsed |
| Skill Extraction | Identify and categorize skills | Profile created |
| Seniority Evaluator | Estimate candidate seniority level | Skills extracted |
| HR Interview | Conduct behavioral HR screening | Interview scheduled |
| Technical Interview | Assess technical knowledge | Interview scheduled |
| Behavioral Interview | Evaluate soft skills and culture fit | Interview scheduled |
| PPE Agent | Conduct pair programming sessions | PPE session started |
| Copilot Agent | Assist recruiters with real-time suggestions | Recruiter action |
| Hiring Recommender | Generate hiring recommendations | Evaluation complete |
| Talent Intel | Market analysis and talent insights | On-demand |
| Workflow Agent | Execute workflow automations | Event triggered |
...
1. **Session Creation** — Candidate selects language and difficulty
2. **Problem Presentation** — AI presents coding challenge
3. **Live Coding** — Candidate writes code in Monaco editor
4. **Real-time Feedback** — AI provides hints and guidance
5. **Code Execution** — Sandbox runs code against test cases
6. **Evaluation** — AI scores based on multiple dimensions
7. **Recommendation** — Hiring recommendation generated
...
| Correctness | 30% | Test cases passed, edge cases |
| Problem Solving | 20% | Approach, algorithm choice |
| Communication | 15% | Explanation, asking questions |
| Time Management | 10% | Pace, efficiency |
...
└─────────────┘     └─────────────┘     └─────────────┘
       │                   │                   │
       │                   │                   │
  emit(event)        partition by key    process(event)
       │                   │                   │
       └───────────────────┴───────────────────┘
                         │
                  ┌──────▼──────┐
...
### 11.2 LLM Provider Routing

```python
# Multi-provider with fallback
providers = [
    OpenAIProvider(model="gpt-4o"),      # Primary
    AnthropicProvider(model="claude-3"),  # Fallback 1
    AzureProvider(model="gpt-4"),         # Fallback 2
]

# Cost-based routing
routing_strategy = {
    "simple_tasks": "gpt-4o-mini",       # Low cost
    "complex_tasks": "gpt-4o",           # High quality
    "creative_tasks": "claude-3",        # Creative output
}
```
...
| Content | Model | Dimensions | Update Frequency |
|---------|-------|------------|------------------|
| Resumes | text-embedding-3-large | 3072 | On upload |
| Job Descriptions | text-embedding-3-large | 3072 | On update |
| Interview Notes | text-embedding-3-small | 1536 | Real-time |
| Knowledge Base | text-embedding-3-large | 3072 | Daily |
...
| recruiter | Candidate, job, interview management |
...
- **EEOC:** Bias monitoring, diversity analytics
...
│  Code   │    │  Image  │    │  Suite  │    │  K8s    │
└─────────┘    └─────────┘    └─────────┘    └─────────┘
     │              │              │              │
     ▼              ▼              ▼              ▼
  GitHub       Docker        Pytest +       ArgoCD
  Actions      Build         Jest           Sync
```
...
| API | Error rate | > 1% |
| AI | Token usage per request | > 4000 |
| AI | Hallucination rate | > 5% |
| PPE | Session completion rate | < 80% |
| Pipeline | Time to hire | > 30 days |
...
- **UUID Primary Keys:** Globally unique, non-sequential
- **JSONB for Flexibility:** Semi-structured data in PostgreSQL
...
| Dashboard | `/dashboard` | Main recruiter workspace |
| Candidates | `/dashboard/candidates` | Candidate management |
| Candidate Detail | `/dashboard/candidates/[id]` | Single candidate view |
| Jobs | `/dashboard/jobs` | Job posting management |
| Interviews | `/dashboard/interviews` | Interview scheduling |
| PPE Sessions | `/dashboard/ppe` | Pair programming sessions |
| PPE IDE | `/ppe/[sessionId]` | Live coding environment |
| AI Interview | `/ai-interview/[sessionId]` | AI interview room |
| Analytics | `/dashboard/analytics` | Metrics and reporting |
| Workflows | `/dashboard/workflows` | Workflow automation |
| Settings | `/dashboard/settings` | System configuration |
| Recruiter | `/dashboard/recruiter` | Recruiter-specific tools |
...
│   ├── (interview)/       # Interview route group
